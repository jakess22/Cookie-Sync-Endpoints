...
def headerContainsSameSiteNone(headers: pd.DataFrame()):
	...
	for header in headers:
		if "SameSite=None" in header or 'samesite=none' in header or 'SameSite=none' in header or 'samesite=None' in header:
...
def getResponseHeaderCookies(response_headers: list[tuple]):
	...
	for cookie_list in header_cookies:
		...
		while i < len(cookie_list):
			regex_1 = re.compile(r'[0-9]{1,2}-[A-Z][a-z]{2}-[0-9]{2,4}') # 14-Jul-2022, 28-Jul-2022
			regex_2 = re.compile(r'[A-Z][a-z]{2}[0-9]{1,2}[A-Z][a-z]{2}[0-9]{2,4}') # Sat15Jul2023
			regex_3 = re.compile(r'[A-Z][a-z]{2}[0-9]{1,2}-[A-Z][a-z]{2}-[0-9]{2,4}') # Thu14-Jul-2022
	...
	common_words = ['Expires', 'expires', 'Path', 'path', 'Check', 'check', 'Permission', 'permission', 'Domain', 'domain', 'doubleclick', 'media' \
	'Secure', 'secure', 'None', 'none', 'Privacy', 'privacy', 'max', 'MAX', 'Max', 'age', 'AGE', 'amazon', 'adsystem', 'secure', 'Secure', 'demdex' \
...
def sharedWithThirdParty(old_req_urls: pd.DataFrame(), new_req_urls: pd.DataFrame()):
	...
	for (index_1, ref_entity), (index_2, req_entity) in zip(ref_entities.iterrows(), req_entities.iterrows()):
...
def getRedirectIDSharingEvents(url_params: pd.Series(), requested_urls: pd.DataFrame(), headers: pd.DataFrame(), shared_with_third_party: list[int]):
	...
	# ids_to_check: possibly new shared id-looking-strings in redirects
	# known_ids: map of id-looking-strings already discovered
	for (i_1, edge_param_ids), (i_2, edge_path_ids), (i_3, edge_loc_header_ids), (i_4, req_url), (i_5, third_party_check) in zip(param_ids.iterrows(), path_ids.iterrows(), location_header_ids.iterrows(), requested_urls.iterrows(), shared_with_third_party_df.iterrows()):
		if third_party_check[0]: # only consider sharing with 3rd parties
			found_param_ids = checkIDisKnown(edge_param_ids, req_url, known_ids) # check if a user cookie_id shared in url parameters, else add to hash
			param_shared_ids.append(found_param_ids)

			found_path_ids = checkIDisKnown(edge_path_ids, req_url, known_ids) # check if a user cookie_id shared in url path, else add to hash
			path_shared_ids.append(found_path_ids)

			found_loc_header_ids = checkIDisKnown(edge_loc_header_ids, req_url, known_ids) # check if a user cookie_id shared in location header, else add to hash
...
def getCookieSyncs(param_shared_ids: list[list[str]], path_shared_ids: list[list[str]], loc_header_shared_ids: list[list[str]], redirect_id_sharing_events: list[int], user_cookies: list[Cookie()], new_req_urls_list: list[tuple]):
	cookie_syncs = []
	endpoint_cs_count = {}

	for (edge_param_id_list, edge_path_id_list, edge_loc_id_list, id_shared, req_url) in zip(param_shared_ids, path_shared_ids, loc_header_shared_ids, redirect_id_sharing_events, new_req_urls_list):
		if id_shared:
			id_found = False
...
redirect_column_names = ['redirect_id_sharing_events', 'url_str_lens', 'req_header_num', 'semicolon_in_query', 'samesite_none_in_header',\
	...
	'special_char_count', 'req_query_str_lens', 'req_query_cookies_num',\
	'tracking_keywords_in_url', 'query_url_check', 'cookie_syncs']


def redirect_extraction(crawl_db, parallelize: bool, progress_bar: bool, verbose: bool, use_memory_fs: bool, entity_map: map):
	...
	if parallelize:
		...
		pandarallel.initialize(progress_bar=progress_bar, verbose=verbose, use_memory_fs=use_memory_fs)
	else:
		pandarallel.initialize(nb_workers=1, progress_bar=progress_bar, verbose=verbose, use_memory_fs=use_memory_fs)
	...
	param_shared_ids, path_shared_ids, loc_header_shared_ids, redirect_id_sharing_events, known_cookies = getRedirectIDSharingEvents(req_query_strs, new_req_urls, headers, shared_with_third_party)
	print(sum(redirect_id_sharing_events), 'ID Sharing events labelled out of ', len(redirect_id_sharing_events), 'redirects\n')

	cookie_syncs, endpoint_cs_count = getCookieSyncs(param_shared_ids, path_shared_ids, loc_header_shared_ids, redirect_id_sharing_events, user_cookies, new_req_urls_list)
	...
	tracking_keywords_next_to_special_char = new_req_urls.parallel_apply(trackingKeywordsNextToSpecialChar)
	...
	data = [redirect_id_sharing_events_df, url_str_lens, req_header_num, semicolon_in_query, samesite_none_in_header,\
		p3p_in_header, etag_in_header, uuid_in_url, tracking_keywords_next_to_special_char, subdomain_check,\
		special_char_count, req_query_str_lens, req_query_cookies_num,\
		tracking_keywords_in_url, query_url_check, cookie_syncs_df]
	redirect_features_df = pd.concat(data, axis=1, keys=redirect_column_names)
	return redirect_features_df



def feature_extraction(crawl_db, parallelize: bool, progress_bar: bool, verbose: bool, use_memory_fs: bool, entity_map: map):
	redirect_features_df = redirect_extraction(crawl_db, parallelize, progress_bar, verbose, use_memory_fs, entity_map)
	return redirect_features_df
