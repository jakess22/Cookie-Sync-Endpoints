...
if __name__ == '__main__':
	...
	if len(sys.argv) > 1: # if parallelization parameters are inputted
		...
		print("[no_par: do not parallelize], [yes_par + no parameters: parallelize with default parameters], [yes_par + 4 parameters: parallelize with custom parameters]")
		print("parameters in order:")
		print("(display progress bar [true/false])")
		print("(verbose [0: don't display logs, 1: display only warning logs, 2: display all logs])")
		print("(memory file system: [None: use if available, True: use, False: use multiprocessing data transfer (pipe) instead])")
	...
	for filename in os.listdir('crawls'):
		...
		if os.path.isfile(crawl_db):
			...
			crawl_df = feature_extraction(crawl_db, parallelize, progress_bar, verbose, use_memory_fs, entity_hash)
